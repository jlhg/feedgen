...
// TestHackmdParser_GetFeed tests feed generation for HackMD
func TestHackmdParser_GetFeed(t *testing.T) {
	t.Parallel()
	parser := HackmdParser{}
...
// TestHackmdParser_ArticleLinks tests HTTP response for article links
func TestHackmdParser_ArticleLinks(t *testing.T) {
	t.Parallel()
	parser := HackmdParser{}
	query := url.Values{}
	query.Set("u", "@BASHCAT")
...
// TestHackmdParser_MissingParameter tests error handling for missing u parameter
func TestHackmdParser_MissingParameter(t *testing.T) {
	t.Parallel()
	parser := HackmdParser{}
	query := url.Values{}
	...
	if err == nil {
		t.Error("Expected error for missing u parameter, got nil")
	}
}
